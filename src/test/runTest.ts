...
async function main() {
	...
	try {
		...
		fixturePath = fs.mkdtempSync(path.join(os.tmpdir(), 'agentic-git-e2e-'));
		...
		auxiliaryPath = fs.mkdtempSync(path.join(os.tmpdir(), 'agentic-git-e2e-aux-'));
		workspaceFilePath = path.join(os.tmpdir(), `agentic-git-e2e-${process.pid}-${Date.now()}.code-workspace`);
		fs.writeFileSync(
			workspaceFilePath,
		...
		run('git config user.name "Agentic Git E2E"');
		...
		revealWorktreeParent = fs.mkdtempSync(path.join(os.tmpdir(), 'agentic-git-reveal-worktree-'));
		...
		process.env.AGENTIC_GIT_REVEAL_WORKTREE_PATH = revealWorktreePath;
		process.env.AGENTIC_GIT_DISABLED_REVEAL_WORKTREE_PATH = disabledRevealWorktreePath;

		// Download VS Code, unzip it and run the integration tests against the fixture workspace.
		// CI normally downloads the requested stable build. Local diagnosis can set AGENTIC_GIT_VSCODE_EXECUTABLE_PATH
		...
		const vscodeExecutablePath = process.env.AGENTIC_GIT_VSCODE_EXECUTABLE_PATH;
		await runTests({
		...
		delete process.env.AGENTIC_GIT_REVEAL_WORKTREE_PATH;
		delete process.env.AGENTIC_GIT_DISABLED_REVEAL_WORKTREE_PATH;
		if (fixturePath) {
