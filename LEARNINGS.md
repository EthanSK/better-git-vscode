...
**Root cause:** VS Code exposes no public API to read or set one repository/group disclosure node. The workaround drove generic `list.*` commands through the workbench's last-focused-list service; Source Control Graph could become that list, commit inputs could steal focus, and repeatedly resetting to an edge made the traversal quadratic as worktree count grew. The original two-repository test asserted SQLite expansion bits and therefore missed the visible many-worktree loop and cross-list targeting failure.
**Fix:** v1.2.31 removes the exact mixed-state restorer, read-only SQLite snapshotting, extension-owned semantic capture, provider mapping, row traversal, and retry/listener paths. New `better-git-vscode.experimentalScmTreeStateManagement` defaults off and hard-gates the manual collapse command plus every startup mutation. With the master off—or on while `collapseWorktreesOnStartup` remains off—activation resolves immediately without an SCM command. The only retained automatic experiment is a double-opt-in, one-shot `workbench.view.scm` followed by VS Code's built-in `workbench.scm.action.collapseAllRepositories` after discovery settles.
**Commit:** pending v1.2.31 release
**Guard:** `scripts/test-scm-state-persistence.mjs` now creates eight repositories/worktrees with staged and working-tree groups, launches the same isolated profile twice in disabled, enabled-inert, and collapse modes, traces Better Git's exact commands at settlement and after a 4.5-second watchdog, rejects production restorer symbols and every generic `list.*` dispatch, and captures each exact numeric window. Disabled and enabled-inert traces stayed `[]`; collapse traces stayed exactly `["workbench.view.scm","workbench.scm.action.collapseAllRepositories"]`. All six proof windows were verified at `-2008,715,1500x1100` on `Built-in Retina Display`, their pixels were inspected, and each isolated window closed while normal VS Code remained untouched. The full real-host suite passed 36/36 after its exact test window was also verified on the MacBook display. TypeScript, webpack, ESLint, `git diff --check`, production archive/manifest/bundle inspection, and byte comparison passed; the tested v1.2.31 VSIX is SHA-256 `73b0a1c22292ccea7a3826f6c2a3d8f5cffa437dfb7965c9237893a9fd0dc2e5`.
---

---
**Date:** 2026-07-16T23:20:00Z
**Trigger:** Ethan 2026-07-16: “I don't have to keep fucking expanding and contracting I want it to save my state essentially”; verify native VS Code first and test the dedicated window on the MacBook Pro display, not the Dell.
**Symptom:** VS Code 1.129 persisted the exact mixed repository and resource-group expansion set in `scm.viewState2`, but after a full restart its asynchronous SCM rebuild reopened nodes that had been saved collapsed. Better Git v1.2.29 independently made this worse by defaulting `collapseWorktreesOnStartup` on, which deliberately erased every mixed repository-header choice.
**Root cause:** Current VS Code reconstructs the SCM `AsyncDataTree` with repositories and groups open before its saved collapsed-node state reliably lands (microsoft/vscode#322318), and extensions have no public per-node expansion API. The persisted IDs use launch-local `scmN` provider handles, so they cannot be trusted across launches without repository-root mapping. Three tempting focus shortcuts are false: `ExtHostSourceControl.selected` follows the repository owning the active editor rather than the repository row focused in the SCM tree; generic `list.select` toggles a row without invoking `SCMViewPane.open()` or changing `scmViewService.focusedRepository`; and `workbench.scm.focus` is conditional. VS Code 1.129's shipped `SCMViewPane.focus()` focuses the first rendered commit-input widget when the tree has no current focus, only falling through to `tree.domFocus()` when no such input is rendered. If it takes the input branch, generic `list.*` commands silently target no SCM tree and can leave every repository header closed while every group remains open.
**Fix:** Better Git v1.2.30 defaults `collapseWorktreesOnStartup` off and, for multi-repository Git-only views on modern desktop hosts, reads the prior `scm.viewState2` and visible-repository order read-only through optional `node:sqlite`. It persists the previous `scmN`-to-root map in extension workspace state, remaps saved repository/group IDs by root, waits for cold Git discovery and VS Code's rebuild race to settle, and restores group bits plus repository headers bottom-to-top without opening an editor. Before every generic row-addressing pass it collapses all repository headers first, waits for the tree operations, and only then invokes `workbench.scm.focus`; with every commit input unrendered, VS Code must take the `tree.domFocus()` path, making the following `list.*` commands deterministic. Mixed non-Git provider views and unavailable/older storage shapes fall back without mutation; an explicit setting value of `true` retains the legacy collapse-all behavior.
**Commit:** 575dfd939455c81e0abf7821e17f003151449bbc (PR #53, squash-merged; published as v1.2.30)
**Guard:** `scripts/test-scm-state-persistence.mjs` creates a real repository plus linked worktree, seeds exactly `repo:scm0`, `resourceGroup:scm0/index`, `resourceGroup:scm1/index`, and `resourceGroup:scm1/workingTree`, launches the same isolated profile twice, and requires the flushed set to be byte-for-byte equivalent after restart. After the final collapse-then-focus correction, three consecutive preservation runs printed `BETTER_GIT_SCM_STATE_PRESERVED`; the explicit legacy override printed `BETTER_GIT_SCM_STATE_RESET_REPRODUCED`. Final manual before/after full-process restart proof used only isolated numeric window IDs at `-2000,720,1500x1100` on the Built-in Retina Display; both the visible tree and flushed `expanded` array matched, with screenshots `scm-state-final-before-restart.png` and `scm-state-final-after-restart.png` in the task visualization directory. Normal VS Code PID 74398 remained untouched. The production VSIX passed archive/manifest/bundle inspection with SHA-256 `9b7eb6eee8c51d0fcdb38e968d1603da679199c7a8e5768328e04dfe714c92ad`; the authenticated publisher check, public validated-only Gallery query, exact package download, and byte comparison all passed when `scripts/verify-marketplace-release.mjs` printed `BETTER_GIT_MARKETPLACE_RELEASE_VERIFIED` for v1.2.30.
---
...
**Fix:** Better Git v1.2.28 contributes `better-git-vscode.open-index-in-system-browser` only when `scmProvider == git`, the resource is local, and `resourceFilename == 'index.html'`; its tolerant handler accepts VS Code's URI, resource-state, or multi-select argument shapes and revalidates the exact local filename before calling `vscode.env.openExternal`. It also contributes `better-git-vscode.copy-worktree-name` only when `scmProviderContext == worktree`, copying `path.basename(sourceControl.rootUri.fsPath)` silently. The misleading local-test executable example in `CONTRIBUTING.md` was corrected from the non-executable `Contents/MacOS/Electron` placeholder to the verified `Contents/MacOS/Code` binary. PR #46 merged the release as `d55503f2111e805dbcef58c9a7d5b0d289ec2076`; Marketplace publication of `EthanSK.better-git-vscode@1.2.28` succeeded, and the exact version-specific gallery package was downloaded with compressed transport, passed archive/manifest inspection, and was byte-identical to the validated upload (`811a19471ce50515e27149f5d60e3e4769bce87d26737527f39c53a393f81143`).
...
**Trigger:** Ethan 2026-07-14: “K clean up everything” after restoring Better Git VS Code
**Symptom:** The temporary `EthanSK.agentic-git@2.0.0` Marketplace listing was still public after the original Better Git VS Code identity had been restored. `@vscode/vsce` 3.9.2 successfully verified the publisher PAT, the same PAT had just published v1.2.27, and the Marketplace role API identified its principal as an Owner, yet `vsce unpublish EthanSK.agentic-git --force` reported that the PAT had expired. The Microsoft accounts already signed into the personal Chrome session both received a publisher-portal 401, so the browser fallback could not safely complete the removal.
**Root cause:** In this incident the Gallery delete endpoint rejected the username-bearing Basic credential produced by VSCE's `getBasicHandler('OAuth', pat)` path, but accepted the identical PAT when Basic auth used an empty username. This was an authentication-shape mismatch, not an expired token or missing publisher role.
**Fix:** With explicit permanent-removal authorization, issue the Gallery `DELETE /_apis/gallery/publishers/EthanSK/extensions/agentic-git?api-version=7.2-preview.2` request using the Keychain PAT as `:<PAT>` Basic credentials without printing it; the endpoint returned HTTP 204. The public Gallery query then returned zero `EthanSK.agentic-git` results while `EthanSK.better-git-vscode` remained `validated, public` at v1.2.27 with the exact subtitle “Fast, keyboard-driven Git review for the agentic age.” The downloaded v1.2.27 Gallery package remained byte-identical to the validated upload (`5e6a81a66fd7e72c6f2fa3ff67b7271c529f72378910faa69ee3eb814178fdd2`). PR #44 is merged as `47a2d693a41f43436d0653b4af98e4f0883e3538`; active user bindings contain only `better-git-vscode.*`, all 16 published commands match their runtime registrations, and final read-only inspection found the normal VS Code installation gallery-sourced at v1.2.27. The temporary renamed VSIX, stale verification worktree, and Gallery downloads were removed.
**Guard:** Treat `verify-pat` success plus an “expired” error from `unpublish` as an authentication-path discrepancy to diagnose, not permission to mint or expose another secret. Confirm the PAT's publisher role and exact target first; only after explicit deletion authorization, retry the narrow Gallery DELETE with an empty Basic username, require HTTP 204, and independently require both a zero-result query for the unwanted identity and a healthy query/hash check for the established identity. Never log the PAT, never guess another browser account, and never mutate normal VS Code as part of release cleanup.
---

---
**Date:** 2026-07-14T22:06:31Z
**Trigger:** Ethan 2026-07-14: restore the original Better Git VS Code name everywhere; keep “Fast, keyboard-driven Git review for the agentic age.” as the Marketplace subtitle
**Symptom:** A brief full rename created a separate Marketplace identity and changed the command/config namespace, repository, local folder, documentation, and Ethan's personal VS Code bindings even though the established product name and listing were preferable. During reversal, the mouse Forward binding reported `command 'agentic-git.smart-forward' not found` even after the files on disk had returned to `better-git-vscode.*`.
**Root cause:** A VS Code Marketplace slug is not cosmetic: `publisher.name` is an immutable extension identity, and contributed command IDs plus user keybindings are another coupled interface. Changing either side alone leaves a live binding that resolves to no registered command. A second cache boundary made the rollback especially confusing: an isolated Extension Development Host had been launched with `--extensionDevelopmentPath` pointing at the original local folder, then the folder was renamed while that host stayed open. The host retained the temporary manifest contribution in memory while its launch path no longer existed, so it could present the stale shortcut but could not load the extension that registered it.
**Fix:** Better Git VS Code v1.2.27 restores `EthanSK.better-git-vscode`, `better-git-vscode.*`, the original runtime/category wording, repository and local-folder names, and Ethan's targeted settings/keybindings while retaining the new agentic-age subtitle and the useful README/navigation documentation. VS Code watches `keybindings.json` live, so restoring F13/F17/F18/F19 and the other targeted bindings required no normal-window reload. The isolated host had no dirty editors; after an advance macOS warning, only that host was restarted against the restored folder path. Normal VS Code remained on the gallery-sourced v1.2.26 installation throughout. The old-to-new deprecation request was visibly withdrawn in [discussion comment 17641159](https://github.com/microsoft/vscode-discussions/discussions/1#discussioncomment-17641159) before release cleanup. The original listing is published and remotely verified before the temporary alternate listing is removed.
**Guard:** For any future Marketplace identity reversal: withdraw pending deprecation first; restore the runtime namespace and targeted user bindings as one compatibility step; before renaming a local repository folder, detect running Extension Development Hosts or other long-lived processes whose launch arguments reference that path and restart only those isolated processes around the rename; publish and download/inspect the established identity; only then remove an unwanted zero-install alternate listing. The identity E2E activates the extension and verifies that every contributed command is present in VS Code's runtime command registry, catching the exact manifest/`registerCommand` mismatch that produces a command-not-found toast. The Marketplace `vspackage` response can be HTTP content-encoded as gzip, so decode it (for example with `curl --compressed`) before comparing its VSIX hash and archive tree. A zero-stale-current-identity search, compile, lint, all 35 isolated-host E2Es, production VSIX inspection, GitHub redirect verification, and untouched normal-VS-Code installation are required.
---
...
**Symptom:** The release workflow treated upgrading Ethan's normal VS Code to the newly published Better Git VS Code version as mandatory closeout, including uninstall/install retries while Marketplace search metadata propagated.
...
**Fix:** Release Better Git VS Code to the Marketplace by default, verify the version-specific remote package, and stop there. Never install, uninstall, update, reload, or restart Better Git VS Code in Ethan's normal VS Code unless he explicitly requests that installation action in the current task; Ethan handles routine Marketplace updates himself.
...
**Trigger:** Ethan 2026-07-14 after v1.2.24/v1.2.25: new-file Next still felt jumpy, left the file too early, Previous reset strangely, large diff hunks had similar edge behavior, and Source Control repository headers reopened after extension restarts
**Symptom:** Option+`>`/`<` could preserve the logical caret but visibly overshoot and return, especially in large or wrapped hunks. Completion rules had diverged between new files and tall hunks; a backward cross-file transition could land at a tall hunk's start and let the next Previous skip it. Repository headers also returned expanded because startup collapse was off by default and its old keep-primary-open path deliberately re-expanded one header by opening a preview file.
**Root cause:** v1.2.22's off-screen forcing-anchor round trip treated an unchanged `revealRange(AtTop)` viewport as a dropped reveal. VS Code source (`ViewLines._computeScrollTopToRevealRange`) shows API reveals reserve `max(editor.cursorSurroundingLines, editor.stickyScroll.maxLineCount)` lines above a target, so `oldTop + 5` can legitimately compute the same physical scroll top when sticky context is five lines. Logical completion was also split between caret and viewport visibility, and built-in `previousChange` lands at a hunk start rather than the bottom required for reverse staged review. The worktree startup path separately layered a preview-tab `scm.autoReveal` re-expand hack over collapse-all even though VS Code exposes no public per-repository expansion-state API.
**Fix:** v1.2.26 removes the forcing anchor and gives both new-file and tall-hunk navigation one caret-owned contract: `clamp(caret ± step)`, consume an exact partial edge step, and permit rollover only on a later press after that edge is visibly presented. Each movement uses one editor-scoped reveal; queued input waits for renderer quiet, upward targets already visible do not scroll down, and wrapped bottom presentation verifies the line-end visual segment while unwrapped editors retain column 0. All backward cross-file paths (ordinary, no-context, and stage-and-Previous) share new-file EOF / diff-last-hunk-bottom landing. Startup collapse defaults on and repeatedly invokes plain collapse-all without re-expanding the primary or opening a file.
**Commit:** c46032e (PR #39, squash-merged; published as v1.2.26)
**Guard:** Real Extension Development Host suite expanded to 35/35. It asserts exact sticky-context L11→L14 rapid progression with step=1, direction reversal from the current caret, full forward/backward hunk-edge consumption before rollover, backward and stage-and-Previous bottom landing, variable-length wrapped SCM-focused rapid input, line-end presentation for wrapped EOF/hunk tails, working-vs-index geometry in a partially staged file, and monotonic visible-range event sequences that fail the old off-screen anchor's overshoot-and-return. TypeScript, webpack, ESLint, production packaging, VSIX tree inspection, and `git diff --check` passed. Marketplace accepted v1.2.26; its version-specific package was downloadable and byte-identical to the validated upload (`123a6aa43e531cc2fbc498e039f984598fdcd7041f8860de56936721be60bcc8`), and normal VS Code finished on the legacy 1.x Marketplace listing at v1.2.26 with `metadata.source: "gallery"`.
---
...
**Commit:** 30803ef (PR #37, squash-merged; published as v1.2.25)
**Guard:** Real-file isolated-host proof with word wrap and SCM focus: L150->L155->L157 stays in file, Previous returns to L152; near the top L12->L7->L2->L1 stays in file. Real-host E2Es cover short-file partial EOF, EOF-visible/caret-not-at-EOF, start-visible/caret-not-at-start, EOF reversal, and rapid final-step+rollover when heavily wrapped EOF is initially off-screen. Full suite 28/28; TypeScript, webpack, and ESLint green. Marketplace's version-specific v1.2.25 package was downloaded and inspected before normal VS Code upgraded the legacy 1.x listing to v1.2.25 with `metadata.source: "gallery"`.
---
...
**Fix:** `src/extension.ts`: keep viewport top/bottom as the source of truth only for visible file-edge detection, but use the caret pinned by the prior Better Git VS Code step as the exact configured ±N progression anchor. Always pin the requested target line rather than substituting `visibleRanges.top`. Preserve the editor-scoped forcing reveal needed when Source Control owns focus; retain one bounded exact-top retry only when `editor.wordWrap=off`, where exact top is attainable and tall-hunk cursor/view coupling needs it. Wrapped editors skip the unattainable second retry, removing the extra flicker.
**Commit:** bafc81a (PR #35, squash-merged; published as v1.2.24)
**Guard:** `src/test/suite/navigation.test.ts` now builds variable-length, sticky-scroll TypeScript fixtures and covers four real-host wrapped/untracked scenarios: SCM-focused exact next/previous, rapid queued repeats plus direction reversals, editor<->SCM focus switching, and custom seven-line jumps under rapid input. Isolated manual result on the actual file is exactly L1->L6->L11->L16->L21->L26 and back to L1 while SCM stays focused. Full real VS Code Extension Development Host suite: 24/24 passing; TypeScript, webpack, and ESLint green. Marketplace accepted v1.2.24; the version-specific gallery package was downloaded and inspected before installation; normal VS Code finished on the legacy 1.x listing at v1.2.24 with `metadata.source: "gallery"`.
---

---
**Date:** 2026-07-14T13:25:40Z
**Trigger:** Ethan 2026-07-14: do not leave Better Git VS Code installed locally; return normal VS Code to the Marketplace version after any necessary local test
...
**Fix:** Added the Release installation policy above. Prefer Extension Development Host testing; if normal VS Code temporarily needs a VSIX, wait until the published version is downloadable, then uninstall the extension and reinstall the exact version by Marketplace ID. Verify the installed-source metadata, not only the displayed version. During this incident the working Marketplace v1.2.22 copy remained installed while guarded installs of the exact legacy 1.x Marketplace ID at v1.2.23 continued; VS Code's gallery index recognized v1.2.23 on check 22 and upgraded it successfully.
**Commit:** a88b75b (v1.2.23 release); docs-only main follow-up adds this installation policy
**Guard:** Release closeout is incomplete until the installed extension row reports the target version without `metadata.source: "vsix"`. The version-specific Marketplace package must be fetched/inspected before uninstalling so a propagation delay cannot strand Ethan without a working extension. Final verification for this incident: legacy 1.x listing at v1.2.23, `metadata.source: "gallery"`, and `autoAddWorktreeOnReveal.default: true`.
---
...
**Commit:** a88b75b (PR #33, squash-merged; published as v1.2.23)
...
**Trigger:** Ethan 2026-07-13: what happened to the Better Git VS Code `+` stage-and-next button; add it back
...
**Commit:** 400d90e
**Guard:** Viewport-derived means reversing direction / short files / exactly-viewport-height files all recompute live per press (no caret-vs-viewport drift possible). Debug logging ('Better Git VS Code' output channel, better-git-vscode.debugLogging) now logs every new-file scroll decision (top/bottom/last/step/advance-vs-step). Smart-mouse forward->goToPreviousDiff flip intentional + untouched; fix lives in shared goToNextDiff/goToPreviousDiff path so keyboard+mouse both get it. tsc+eslint+webpack green. Published v1.2.15.
---
