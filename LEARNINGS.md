...
- Agentic Git code fixes requested by Ethan include completing the Marketplace release by default unless he explicitly says not to release that change.
- Release work must **not install, uninstall, update, reload, or restart** Agentic Git in Ethan's normal VS Code. Ethan will install Marketplace updates himself after they appear.
...
---
**Date:** 2026-07-14T21:28:01Z
**Trigger:** Ethan 2026-07-14: rename the extension, repository, folders, Marketplace identity, documentation, links, and social references to Agentic Git; document the completed navigation fix
**Symptom:** A display-name-only rename would leave the Marketplace slug, command/config namespaces, personal shortcuts, repository URL, output strings, test fixture prefixes, documentation, and public links under the superseded identity. Changing a VS Code extension's manifest `name` also creates a different Marketplace identity, so existing users do not receive the renamed extension as an automatic update.
**Root cause:** VS Code extension identity is the immutable `publisher.name` pair, while GitHub repository identity and local folder names are separate layers. A complete product rename therefore crosses code, user configuration, packaging, distribution, source hosting, and public-profile surfaces; treating any one layer as cosmetic leaves broken shortcuts or stale branding.
**Fix:** Agentic Git v2.0.0 moves the manifest, command IDs, settings, runtime strings, tests, README, changelog, and current learning references to `agentic-git`; Ethan's explicit VS Code settings/keybindings are migrated without restarting or installing anything. The GitHub repository is renamed so GitHub preserves redirects, the new `EthanSK.agentic-git` Marketplace identity is published, and the 1.x listing is retained as a deprecation/migration pointer instead of being unpublished. The caret-owned v1.2.26 behavior is preserved and explained in `docs/navigation-behavior.md`.
**Guard:** A pre-release zero-stale-identity search must pass across tracked current files and Ethan's active VS Code settings/keybindings. Compile, lint, all 35 real isolated-host E2Es, a production VSIX identity/tree inspection, GitHub old/new URL verification, and a version-specific Marketplace package download are required. Never install, reload, or restart Ethan's normal VS Code during this release.
---
...
**Symptom:** The release workflow treated upgrading Ethan's normal VS Code to the newly published Agentic Git version as mandatory closeout, including uninstall/install retries while Marketplace search metadata propagated.
...
**Fix:** Release Agentic Git to the Marketplace by default, verify the version-specific remote package, and stop there. Never install, uninstall, update, reload, or restart Agentic Git in Ethan's normal VS Code unless he explicitly requests that installation action in the current task; Ethan handles routine Marketplace updates himself.
...
**Fix:** `src/extension.ts`: keep viewport top/bottom as the source of truth only for visible file-edge detection, but use the caret pinned by the prior Agentic Git step as the exact configured ±N progression anchor. Always pin the requested target line rather than substituting `visibleRanges.top`. Preserve the editor-scoped forcing reveal needed when Source Control owns focus; retain one bounded exact-top retry only when `editor.wordWrap=off`, where exact top is attainable and tall-hunk cursor/view coupling needs it. Wrapped editors skip the unattainable second retry, removing the extra flicker.
...
**Trigger:** Ethan 2026-07-14: do not leave Agentic Git installed locally; return normal VS Code to the Marketplace version after any necessary local test
...
**Trigger:** Ethan 2026-07-14: Shift+Cmd+E reveal does not work for worktree files; modify Agentic Git to support it
**Symptom:** `agentic-git.reveal-current-file-in-explorer` opened/resolved worktree files but could not select them when their linked worktree appeared in Source Control without being an Explorer workspace folder.
...
**Fix:** v1.2.23: reveal now opens the editable file first, resolves the owning worktree with the existing git-API/`.git`-walk logic, subscribes for the workspace-folder event before adding the missing root through one shared add helper, waits for Explorer, then calls `revealInExplorer`. Opening first preserves the target across VS Code's required single-folder -> multi-root host restart. The explicit add-worktree command reuses the same helper. `agentic-git.autoAddWorktreeOnReveal` gates the mutation and defaults true; when false the command opens the file, leaves workspace folders untouched, and explains why no Explorer node can be selected.
...
**Trigger:** Ethan 2026-07-13: what happened to the Agentic Git `+` stage-and-next button; add it back
...
**Fix:** src/extension.ts v1.2.18: shared top-and-pin movement now uses VS Code's real relative logical-line `editorScroll`, awaits the exact editor/view-column viewport event (short timeout fallback), then pins the caret. Both new-file and tall-hunk up/down paths await it. All shared next/previous navigation is serialized through one queue so keyboard repeat and smart-mouse presses cannot overlap against stale state or disposed editors. src/test now supports `AGENTIC_GIT_VSCODE_EXECUTABLE_PATH` for local real-host runs and uses production-height fixtures.
**Commit:** 7745dd2
...
**Commit:** 400d90e
**Guard:** Viewport-derived means reversing direction / short files / exactly-viewport-height files all recompute live per press (no caret-vs-viewport drift possible). Debug logging ('Agentic Git' output channel, agentic-git.debugLogging) now logs every new-file scroll decision (top/bottom/last/step/advance-vs-step). Smart-mouse forward->goToPreviousDiff flip intentional + untouched; fix lives in shared goToNextDiff/goToPreviousDiff path so keyboard+mouse both get it. tsc+eslint+webpack green. Published v1.2.15.
---
...
**Trigger:** Mini keyboard-emulation test on v1.2.9, evidence /tmp/agentic-git-tall-hunk-evidence-129/ (2026-07-04)
...
**Root cause:** The editor/title menu pointed at agentic-git.stage-current-file (stage, no navigate); there was no mouse path that both staged and advanced, and no memory of which direction (next/previous) he was reviewing in.
**Fix:** src/extension.ts: added module-level lastNavDirection ('next'|'previous', default 'next') set by next/previous-scm-change, next/previous-changed-file, and stage-and-next/previous (NOT the smart mouse cmds — their in-diff direction is intentionally flipped). New command agentic-git.stage-current-file-and-advance calls the SAME stageCurrentFileAndAdvance(lastNavDirection) that the Shift+Alt+./,keyboard shortcuts use. package.json editor/title menu repointed from stage-current-file to stage-current-file-and-advance (kept $(add) icon + gitOpenRepositoryCount!=0 when-clause). stage-current-file stays registered for anyone who bound it.
...
**Symptom:** agentic-git: next/previous SCM change (the headline > and < keys, alt+. / alt+,) were BROKEN on QWERTY — > navigated BACKWARD through diff changes, < navigated FORWARD, and outside a diff they did editor history navigation instead of change navigation. Dvorak layout was PERFECT (source of truth).
...
**Trigger:** Ethan request: 'make agentic-git minimize the worktrees if there are any when opening / reloading window'
...
**Fix:** Added isFullyAddedFile() (git-status detection: UNTRACKED/INDEX_ADDED/INTENT_TO_ADD via git API + toFilePathUri) + stepThroughNewFile(); goToNextDiff/goToPreviousDiff now step cursor down/up newFileNavLineJump lines (default 5) through a fully-added file, falling through to next/prev FILE at the edge. New setting agentic-git.newFileNavLineJump. v1.2.0
**Commit:** 5e3ea20
...
**Trigger:** Add Dvorak mode toggle to EthanSK/agentic-git
**Symptom:** Dvorak users had to hand-maintain a keybindings.json override block to remap agentic-git nav keys (alt+v/alt+w etc) and disable the QWERTY defaults via -command minus-entries
...
**Fix:** Added boolean setting agentic-git.dvorakMode; each of the 4 swapped commands ships two contributes.keybindings entries gated with VS Code native config.<settingId> when-clauses. Toggle disables old + enables new in one step, no -command entries, no setContext, no restart. User keybindings still win.
**Commit:** d67704f
...
**Symptom:** Productizing the VS Code fork: rename to public name 'Agentic Git' without breaking Ethan's keyboard shortcuts
...
**Trigger:** productize for marketplace publish under publisher ethansk renamed agentic-git
