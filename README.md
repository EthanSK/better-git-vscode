...
Some hunks **run below the visible screen.** One press of next-change can land near the bottom — and the rest runs off-screen, so you'd have to take your hands off the keyboard, scroll, then press next again. Better Git checks the hunk's actual first and final rendered positions: a newly reached hidden tail is lifted into view immediately, and if the full hunk still cannot fit, the **same key steps through it in stages.** The final partial step always lands on and presents the exact hunk end (including the last visual segment of a wrapped line); only the following press moves on. `previous-change` mirrors it at the hunk start. Reverse direction any time and it continues from the current caret instead of teleporting.

Hunks whose complete rendered range is already visible are untouched — one press still jumps straight to the next/previous hunk. Auto staging keys off what is actually presented, so it adapts to window zoom, wrapping, sticky context, diff alignment, and where the hunk landed instead of guessing from line counts. Each step keeps a few lines of overlap so you never lose your place. Tune it with `better-git-vscode.hunkStagingThreshold`, `hunkStagingLineStep`, and `hunkStagingOverlap`, or turn it off with `hunkStagingEnabled`.
...
- **Tall-hunk staging** — present or step through any hunk whose complete rendered range is not on-screen, instead of letting unread lines run off the bottom (`better-git-vscode.hunkStagingEnabled`, default on — see *Step through tall hunks in stages* above). Tune the engage threshold (`hunkStagingThreshold`, 0 = auto/rendered visibility), the per-step scroll (`hunkStagingLineStep`, 0 = auto), and the overlap kept between steps (`hunkStagingOverlap`, default 4).
